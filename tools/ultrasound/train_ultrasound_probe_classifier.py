#!/usr/bin/env python3
"""Train ultrasound probe classifier (fss_id_probe) from HDMI frames."""
...
class ProbeDataset(Dataset):
    ...
    def __getitem__(self, idx: int):  # type: ignore[override]
        ...
        target = self.label_to_idx[row.probe_id]
        metadata = {
...
def make_loader(
    ...
    dataset = ProbeDataset(rows=rows, label_to_idx=label_to_idx, image_size=image_size, augment=augment)
    ...
    if augment:
        if use_balanced_sampler:
            ...
            sample_weights = [1.0 / counts[row.probe_id] for row in rows]
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Train probe classifier (fss_id_probe) from ultrasound HDMI frames."
    )
    parser.add_argument(
        "--manifest",
        type=Path,
        default=Path("artifacts/20_datasets/vendor_template_dataset_v1/manifest_vendor_template_no_negative.csv"),
        help="Manifest CSV con colonna fss_id_probe (senza negativi).",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("artifacts/30_models/probe_training"),
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Se attivo, rimuove i campioni val/test con probe_id non presenti in train.",
    )
    return parser
...
def _write_per_class_csv(path: Path, per_class: Sequence[Dict[str, object]]) -> None:
    with path.open("w", encoding="utf-8", newline="") as fh:
        ...
        writer.writerow(["probe_id", "precision", "recall", "f1", "support"])
...
def _drop_unseen(rows: List[SampleRow], known_labels: Sequence[str]) -> List[SampleRow]:
    ...
    return [row for row in rows if row.probe_id in known]
...
def main() -> int:
    ...
    probe_train = sorted({row.probe_id for row in split_rows["train"]})
    label_to_idx = {label: idx for idx, label in enumerate(probe_train)}
    idx_to_label = probe_train

    unseen_val = sorted({row.probe_id for row in split_rows["val"] if row.probe_id not in label_to_idx})
    unseen_test = sorted({row.probe_id for row in split_rows["test"] if row.probe_id not in label_to_idx})
    dropped_stats = {
    ...
    if unseen_val or unseen_test:
        if not args.drop_unseen_val_test:
            raise RuntimeError(
                "Classi probe presenti in val/test ma assenti in train. "
        ...
        print(
            "ATTENZIONE: filtrati campioni val/test con probe_id unseen in train. "
    ...
    print(f"Probe classes (train): {len(idx_to_label)}", flush=True)
    ...
    model = ProbeClassifier(num_classes=len(idx_to_label), pretrained=args.pretrained).to(device)
    ...
    per_class_path = output_dir / "test_per_probe_metrics.csv"
    ...
    print(f"Metriche per probe: {per_class_path}", flush=True)
